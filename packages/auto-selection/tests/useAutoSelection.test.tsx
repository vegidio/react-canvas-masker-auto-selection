...
describe('useAutoSelection — mode state', () => {
  it('starts in paint mode and toggles to auto', () => {
    ...
    const { result } = renderHook(() => useAutoSelection({ canvasRef: ref, source: null }));
  ...
  it('respects initialMode', () => {
    ...
    const { result } = renderHook(() =>
      useAutoSelection({ canvasRef: ref, source: null, initialMode: 'auto' }),
    );
    expect(result.current.mode).toBe('auto');
  });
});
...
describe('useAutoSelection — no backend', () => {
  it('detectAt throws when `sam` is not provided', async () => {
    ...
    await expect(result.current.detectAt({ x: 1, y: 1 })).rejects.toThrow(/no backend configured/i);
  });
});
