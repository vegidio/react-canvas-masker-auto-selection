...
export function useAutoSelection(options: AutoSelectionOptions): AutoSelectionResult {
  ...
  const engineRef = useRef<SamEngineLike | null>(null);
  ...
  const ensureEngine = useCallback(async (): Promise<SamEngineLike | null> => {
    if (!sam || !samKey) return null;
  ...
  const detectAt = useCallback(
    async (point: ImagePoint): Promise<DetectedObject | null> => {
      setError(null);
      if (!sam) {
        const err = new Error(
  ...
  const handleOverlayPick = useCallback(
    async (clientPoint: ClientPoint) => {
      ...
      const imagePoint = clientToImagePoint(clientPoint.clientX, clientPoint.clientY, maskCanvas);
